...
async def list_plans(_: dict = Depends(get_current_user)):
    return await db.benefit_plans.find({}, {"_id": 0}).sort("name", 1).to_list(200)
...
async def delete_plan(pid: str, user: dict = Depends(require_admin)):
    await db.benefit_plans.delete_one({"id": pid})
    await db.benefit_enrollments.delete_many({"plan_id": pid})
...
async def list_enrollments(user: dict = Depends(get_current_user)):
    ...
    plans = {p["id"]: p for p in await db.benefit_plans.find({}, {"_id": 0}).to_list(200)}
...
async def enroll(body: EnrollmentIn, user: dict = Depends(get_current_user)):
    ...
    plan = await db.benefit_plans.find_one({"id": body.plan_id}, {"_id": 0})
    if not plan:
        raise HTTPException(404, "Plan not found")
    existing = await db.benefit_enrollments.find_one({"employee_id": eid, "plan_id": body.plan_id})
    if existing:
...
async def unenroll(enr_id: str, user: dict = Depends(get_current_user)):
    enr = await db.benefit_enrollments.find_one({"id": enr_id}, {"_id": 0})
    if not enr:
        raise HTTPException(404, "Not found")
    if user["role"] != "admin" and enr["employee_id"] != user.get("employee_id"):
        raise HTTPException(403, "Forbidden")
    await db.benefit_enrollments.delete_one({"id": enr_id})
