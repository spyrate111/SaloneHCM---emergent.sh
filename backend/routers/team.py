...
async def _team_summary(manager_eid: str, tf: dict) -> dict:
    """Compute team metrics for a given manager's employee_id."""
    reports = await db.employees.find({"manager_id": manager_eid, **tf}, {"_id": 0}).to_list(500)
    ...
    if report_ids:
        pending_leaves = await db.leave_requests.count_documents(
            {"employee_id": {"$in": report_ids}, "status": "pending", **tf}
        )

    recent_leaves = []
    if report_ids:
        recent_leaves = await db.leave_requests.find(
            {"employee_id": {"$in": report_ids}, **tf}, {"_id": 0}
...
async def my_team(user: dict = Depends(get_current_user)):
    ...
    return await _team_summary(eid, tenant_filter(user))


@router.get("/managers")
async def list_managers(user: dict = Depends(require_admin)):
    """All employees flagged is_manager — useful for the Employee form dropdown."""
    return await db.employees.find(
        {"is_manager": True, **tenant_filter(user)},
        {"_id": 0, "id": 1, "first_name": 1, "last_name": 1, "department": 1, "job_title": 1},
    ).sort("first_name", 1).to_list(500)


@router.get("/{eid}")
async def team_for(eid: str, user: dict = Depends(require_admin)):
    tf = tenant_filter(user)
    e = await db.employees.find_one({"id": eid, **tf}, {"_id": 0})
    if not e:
        raise HTTPException(404, "Employee not found")
    summary = await _team_summary(eid, tf)
