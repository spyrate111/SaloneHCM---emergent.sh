"""What-if payroll simulator + saved scenarios + Decision Brief PDF."""
import io
import uuid
...
from core import db, require_admin, audit, now_utc, iso, tenant_filter, with_tenant, require_feature
from payroll_engine import calc_payslip
from . import _decision_brief  # see helpers module below

router = APIRouter(
    prefix="/payroll",
    tags=["payroll-sim"],
    dependencies=[Depends(require_feature("simulator"))],
)
...
async def simulate(body: SimIn, user: dict = Depends(require_admin)):
    return await _do_simulate(body, user)


async def _do_simulate(body: SimIn, user: dict) -> dict:
...
async def list_scenarios(user: dict = Depends(require_admin)):
    return await db.payroll_scenarios.find(tenant_filter(user), {"_id": 0}).sort("created_at", -1).to_list(200)


@router.get("/scenarios/{sid}")
async def get_scenario(sid: str, user: dict = Depends(require_admin)):
    s = await db.payroll_scenarios.find_one({"id": sid, **tenant_filter(user)}, {"_id": 0})
    if not s:
        raise HTTPException(404, "Not found")
    rules = [SimRule(**r) for r in s["rules"]]
    sim = await _do_simulate(SimIn(rules=rules), user)
...
async def decide_scenario(sid: str, body: ScenarioDecision, user: dict = Depends(require_admin)):
    ...
    s = await db.payroll_scenarios.find_one({"id": sid, **tf}, {"_id": 0})
    if not s:
        raise HTTPException(404, "Not found")
...
async def _do_apply_scenario(sid: str, user: dict) -> dict:
    ...
    rules = [SimRule(**r) for r in s["rules"]]
    emps = await db.employees.find({"status": "active", **tf}, {"_id": 0}).to_list(2000)
    changed = []
    for e in emps:
...
async def delete_scenario(sid: str, user: dict = Depends(require_admin)):
    res = await db.payroll_scenarios.delete_one({"id": sid, **tenant_filter(user)})
...
async def compare_scenarios(body: CompareIn, user: dict = Depends(require_admin)):
    return await _do_compare(body, user)


async def _do_compare(body: CompareIn, user: dict) -> dict:
    tf = tenant_filter(user)
    rows = []
...
async def compare_pdf(body: CompareIn, user: dict = Depends(require_feature("decision_brief_pdf"))):
    """Decision Brief PDF — board-meeting-ready summary of compared scenarios."""
    data = await _do_compare(body, user)
    company = await db.companies.find_one({"id": user["company_id"]}, {"_id": 0})
    pdf_bytes = _decision_brief.build_pdf(data, user, company)
    await audit("decision_brief_export", "payroll/scenarios/compare", user, {"scenarios": len(data["rows"])})
    fname = f"SaloneHCM-Decision-Brief-{datetime.now(timezone.utc).strftime('%Y%m%d')}.pdf"
