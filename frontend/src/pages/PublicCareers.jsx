  ...
  if (loading && !data) {
    return (
      <div className="min-h-screen grid place-items-center bg-[#F7F6F2]">
        <Loader2 className="w-6 h-6 animate-spin text-[#133326]" />
      </div>
    );
  }
...
function Header({ org, totalOpen }) {
  return (
    <header className="bg-[#133326] text-white">
    ...
    <div className="flex items-center gap-2 flex-wrap" data-testid="careers-filters">
      <div className="relative flex-1 min-w-[240px]">
        ...
        <input
          ...
          className="w-full pl-10 pr-3 h-11 bg-white border border-[#E2DFD6] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#133326]"
...
function PostingCard({ posting, onOpen }) {
  return (
    <button
      ...
      className="w-full text-left bg-white border border-[#E2DFD6] hover:border-[#133326] rounded-lg p-5 transition group"
    >
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-heading text-xl font-bold mt-1 text-[#0F2C24] group-hover:text-[#133326]">{posting.title}</h3>
        ...
        <ChevronRight className="w-5 h-5 text-[#9aa0a6] group-hover:text-[#133326]" />
      </div>
    </button>
  );
}
...
function ApplyPanel({ slug, pid, onBack }) {
  ...
  if (loading) return <div className="min-h-screen grid place-items-center bg-[#F7F6F2]"><Loader2 className="w-6 h-6 animate-spin text-[#133326]" /></div>;
  if (!posting) return null;

  return (
    <div className="min-h-screen bg-[#F7F6F2]" data-testid="careers-apply-page">
      <div className="bg-[#133326] text-white">
...
function PostingHeader({ posting }) {
  return (
    <div className="bg-white border border-[#E2DFD6] rounded-lg p-6" data-testid="careers-posting-detail">
      ...
      <h1 className="font-heading text-3xl font-bold mt-2 text-[#0F2C24]">{posting.title}</h1>
    ...
    <form onSubmit={onSubmit} className="mt-6 bg-white border border-[#E2DFD6] rounded-lg p-6 space-y-4" data-testid="careers-apply-form">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="Full name">
          <input required minLength={2} maxLength={160} value={form.name} onChange={upd("name")}
            className="w-full h-11 px-3 bg-white border border-[#E2DFD6] rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#133326]"
        ...
        <Field label="Phone number">
          <input required minLength={5} value={form.phone} onChange={upd("phone")}
            ...
            className="w-full h-11 px-3 bg-white border border-[#E2DFD6] rounded-md text-sm font-data focus:outline-none focus:ring-2 focus:ring-[#133326]"
      ...
      <Field label="Resume summary — highlight your qualifications, experience, and why you're a fit">
        <textarea required minLength={20} maxLength={4000} rows={7} value={form.resume_summary} onChange={upd("resume_summary")}
          ...
          className="w-full px-3 py-2 bg-white border border-[#E2DFD6] rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#133326]"
      ...
      <div className="pt-2 border-t border-[#F1EEE6] flex items-center justify-end">
        <button
          type="submit"
          disabled={submitting}
          className="inline-flex items-center gap-2 bg-[#133326] hover:bg-[#0F281E] disabled:opacity-60 text-white text-sm px-5 py-2.5 rounded-md"
...
function SuccessPanel({ result, onBack }) {
  return (
    <div className="bg-white border border-[#C9E2D2] rounded-lg p-8 text-center" data-testid="careers-apply-success">
      <div className="w-14 h-14 rounded-full bg-[#E6F4EC] grid place-items-center mx-auto">
        <CheckCircle2 className="w-6 h-6 text-[#2D7A5D]" />
      </div>
      ...
      <div className="mt-5 inline-flex items-center gap-2 bg-[#F7F6F2] border border-[#E2DFD6] rounded-md px-4 py-2 text-xs text-[#525860]">
        Reference number: <code className="font-data text-[#133326] font-semibold" data-testid="careers-application-ref">{result.application_ref}</code>
      </div>
      <div className="mt-6">
