  ...
  return (
    <div className="bg-white border-2 border-[#26547C] rounded-lg p-6" data-testid="comparison-panel">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
        <Callout color="bg-[#E6F4EC] border-[#9CC8B1] text-[#2D7A5D]" label="💰 Cheapest" title={cheapest?.scenario.title} />
...
function ComparisonRow({ index, r, isWinner, isTargeted }) {
  ...
  const deltaClass = positive ? "text-[#B84F2F]" : "text-[#2D7A5D]";
  const sign = positive ? "+" : "";
  return (
    <tr className={`border-t border-[#E2DFD6] ${isWinner ? "bg-[#F4FBF5]" : ""}`}>
