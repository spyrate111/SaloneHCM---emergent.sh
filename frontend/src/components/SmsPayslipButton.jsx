...
export default function SmsPayslipButton({ run }) {
  ...
  const send = async (dry) => {
    ...
    try {
      ...
      const detail = e?.response?.data?.detail || "SMS send failed";
      toast.error(detail);
    } finally {
      setBusy(false);
    }
          ...
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto" data-testid="sms-modal">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h3 className="font-heading text-xl font-semibold mt-1">Payslip SMS — {run.period}</h3>
              ...
              <button onClick={() => !busy && setOpen(false)} className="text-[#525860]"><X className="w-4 h-4" /></button>
            </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {twilioConfigured && (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => { if (window.confirm(`Send live SMS to ${run.totals.employee_count} employees? This will incur Twilio charges.`)) send(false); }}
                  ...
                  >
                    <MessageSquare className="w-3.5 h-3.5" /> {busy ? "Sending…" : `Send live to ${run.totals.employee_count} employees`}
                  </button>
                )}
              </div>
            )}
            ...
            {result && (
              <div data-testid="sms-result" className="space-y-3">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button onClick={() => { setResult(null); }} className="text-sm px-4 py-2 border border-[#E2DFD6] rounded-md">Run again</button>
                  <button onClick={() => setOpen(false)} className="text-sm bg-[#133326] text-white px-4 py-2 rounded-md">Close</button>
                </div>
              </div>
            )}
          </div>
