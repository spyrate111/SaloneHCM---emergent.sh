...
export default function TwoFactorCard() {
  ...
  const load = () => api.get("/auth/2fa/policy").then((r) => setPolicy(r.data));
  useEffect(() => { load(); }, []);
  ...
  const disable2fa = async (ev) => {
    ev.preventDefault();
    setBusy(true);
    try {
      await api.post("/auth/2fa/disable", { password: disablePwd, code: disableCode });
  ...
  return (
    <section data-testid="twofa-card" className="bg-white border border-[#E2DFD6] rounded-xl overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h2 className="font-heading text-2xl font-bold mt-1 flex items-center gap-2">
              ...
              {enabled
                ? <span className="text-[10px] uppercase tracking-wider font-medium px-2 py-1 rounded-full bg-[#E6F4EC] text-[#2D7A5D] inline-flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Enabled</span>
                : <span className="text-[10px] uppercase tracking-wider font-medium px-2 py-1 rounded-full bg-[#EBE8E0] text-[#525860] inline-flex items-center gap-1"><ShieldOff className="w-3 h-3" /> Disabled</span>}
            </h2>
          ...
          {required && !enabled && (
            <div className="text-xs bg-[#FBE9DF] border border-[#E8B89C] text-[#8B3A1C] rounded-md px-3 py-2 inline-flex items-center gap-1.5 max-w-sm">
              ...
              Required for {policy.role} role — enable to protect your tenant.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="px-7 py-6">
        ...
        {enabled && disabling && (
          <form onSubmit={disable2fa} className="space-y-3 max-w-md" data-testid="twofa-disable-form">
            <p className="text-sm text-[#525860]">Confirm your password and a current 2FA code to disable.</p>
            <input required type="password" value={disablePwd} onChange={(e) => setDisablePwd(e.target.value)} placeholder="Current password" className="w-full bg-white border border-[#E2DFD6] rounded-md px-3 py-2.5 text-sm font-data" />
            <input required inputMode="numeric" pattern="\d{6}" maxLength={6} value={disableCode} onChange={(e) => setDisableCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="6-digit code" className="w-full bg-white border border-[#E2DFD6] rounded-md px-3 py-2.5 text-sm font-data tracking-widest" />
            <div className="flex gap-2">
              <button type="button" onClick={() => setDisabling(false)} className="text-sm px-4 py-2 border border-[#E2DFD6] rounded-md">Cancel</button>
