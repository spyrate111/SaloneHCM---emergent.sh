...
## v1.34 (Aug 2026) — OOZ alerts + digest, Training progress, Payslip QR verify, Leave calendars
- **Out-of-zone punch alerts** (`/app/backend/ooz_alerts.py`): punch now stores `in_zone`; when False, a background task pushes (web push) + SMSes the branch supervisor and pushes tenant admins (no SMS). Alerts logged in `ooz_alerts`; daily 18:00 UTC email digest to tenant admins (idempotent per company/day via `ooz_digests`, attached in scheduler.py). Admin endpoints: `GET /api/mobile/ooz-alerts?date=` and `POST /api/mobile/ooz-digest/run-now` (SENDS REAL EMAIL via Resend). Verified live: digest email accepted by Resend.
- **Training progress** (`routers/training_progress.py`, collection `training_progress` keyed user_id+base_slug): `POST /complete` (fired by mobile video `ended`), `GET /me`, `GET /team?branch_id=` (admin: tenant-wide; branch supervisor: own branches; others 403). Mobile Training screen: my-progress bar, green "Completed" badges, supervisor "Team progress" expandable rows. Desktop: `TrainingProgressPanel.jsx` mounted on Talent → Learning (V1-V7 check matrix + branch filter).
- **Payslip QR verify**: `build_payslip_pdf` gained `verification_id`/`verify_url` params rendering a QR + "Bank verification" block; `payslip_pdf` endpoint upserts a stable `payslip_verifications` doc per (run_id, employee_id). Public no-auth `GET /api/public/payslip/{vid}` + page `/verify-payslip/:vid` (`VerifyPayslip.jsx`) — genuine/not-found cards.
- **Leave calendars**: `GET /api/leave/calendar?month=YYYY-MM` (admin: tenant; manager: self+direct reports; plain employees 403; bad month 422). Desktop month grid `LeaveCalendar.jsx` on /leave (name chips green approved / amber pending, +n more, month arrows); mobile compact `TeamCalendar` in MobileLeave.jsx (count badges, tap-day detail). Shared helpers `lib/leaveCal.js`.
- **Tests**: `/app/backend/tests/test_iter36_alerts_progress_verify_calendar.py` (12 tests). Full suite **480 passed, 7 skipped**. Testing agent frontend: **5/5 PASS** (`/app/test_reports/iteration_35.json`; the first run iteration timed-out with no report — scope was trimmed and rerun).
- **CRITICAL LESSON (recorded)**: NEVER issue two search_replace edits to the SAME file in one parallel batch — later writes clobber earlier ones (bit payroll_engine.py, MobileTraining.jsx, Talent.jsx this session). One edit per file per batch, or sequential.

## v1.35 (Aug 2026) — Snooze rules, leave conflict warning, training reminders, payslip scan log
- **Alert snooze rules**: `ooz_snoozes` CRUD under `/api/mobile/ooz-snoozes` (admin-only; POST/GET/PATCH/DELETE, date-range + reason validation). Active snooze → `fire_out_of_zone_alert(..., snooze=)` logs the alert with `snoozed=True` + reason and suppresses push/SMS/admin push; daily digest and its company-scan exclude `snoozed` alerts. Desktop `OozSnoozePanel.jsx` (table + create/edit modal + delete-with-confirm) mounted under the punch map on /attendance (admins only). ooz_alerts.py was REWRITTEN whole-file (safer than multi-edit).
- **Leave conflict warning**: `GET /api/leave/{lid}/conflicts` (admin or the employee's manager) — warns when approving puts ≥ max(2, ceil(20% of branch active staff)) off any overlapping day; returns per-day off_count + already-approved names; counts APPROVED overlaps only; never blocks. Desktop /leave shows an amber "Coverage warning" dialog (Cancel / Approve anyway); mobile /m/leave shows a bottom sheet. Mobile "Waiting on you" now filters to pending only.
- **Weekly training reminders** (`training_reminders.py`, cron Mon 09:00 UTC in scheduler.py): push-only (title+body localized en/krio/mende/temne via `users.training_lang`, saved by `POST /api/training-progress/lang` fired from the mobile lang toggle), skips staff who completed all videos, idempotent per user per ISO week via `training_reminders` collection. Manual admin trigger `POST /api/training-progress/reminders/run-now`.
- **Payslip scan log**: every public `GET /api/public/payslip/{vid}` lookup logs `payslip_scans` (ts, ip from x-forwarded-for, UA). Admin `GET /api/payroll/verification-scans` (+`/{vid}` history) — flag = 5+ scans within ANY sliding 24h window. Desktop `PayslipScanPanel.jsx` on /payroll: counts, first/last scan, red "Unusual" badge, click-row expandable history.
- **Tests**: `tests/test_iter37_snooze_conflict_reminders_scans.py` (8 tests). Full suite **488 passed, 7 skipped**. Testing agent frontend: **4/4 PASS** (`/app/test_reports/iteration_36.json`).
- Backlog note from review (P2): PayslipScanPanel pagination for high-scan tenants.

## v1.36 (Aug 2026) — iter38 feature pack complete: Fraud Alert Push, Snooze From Alert, Coverage Planner, Reminder Nudge Stats
- **Feature 1 — Fraud Alert Push** (done in prior fork): 5+ payslip QR scans/24h → one deduped alert (`payslip_scan_alerts`) + web push to all tenant admins deep-linking to /payroll scan log.
- **Feature 2 — Snooze From Alert**: OOZ push notification carries a "Snooze alerts for this employee" action (sw.js) deep-linking to `/m/clock?snooze={eid}&name=`; MobileClock renders `QuickSnoozeSheet` (from/to default today, reason) posting to `POST /api/mobile/ooz-snoozes`. Backend RBAC extended: branch supervisors may snooze their OWN branch staff (admins any). **UI BUG FOUND + FIXED**: Leaflet map panes (z-index 400–1000) rendered ON TOP of the sheet (Tailwind z-50) — save clicks actually hit the map's OSM attribution link and navigated away. Fix: sheet uses `z-[1100]`. Verified E2E via screenshot (deep link → sheet → save → success toast).
- **Feature 3 — Coverage Planner**: new `GET /api/leave/coverage-preview?start_date&end_date[&employee_id]` — any employee previews their OWN branch coverage while drafting a leave request (admins can pass employee_id); returns per-day off_count/already_off/breach vs threshold max(2, ceil(20% branch)); approved overlaps only; 422 on bad dates. Live widget in BOTH leave forms: desktop /leave modal (`leave-coverage-preview`) and mobile /m/leave sheet (`mobile-leave-coverage`) — green "Coverage looks fine" or amber breach-day list ("You can still submit — approver sees this too"). Verified via screenshots in breach state on both.
- **Feature 4 — Reminder Nudge Stats**: new `GET /api/training-progress/reminders/stats` (admin-only, 403 others) — groups `training_reminders` by ISO week: reminded, delivered (push_sent), completed_after (any video completion within 7 days after the nudge), nudge_rate %; totals across last 12 weeks. Desktop `ReminderNudgeStats.jsx` mounted on Talent → Learning below TrainingProgressPanel: 3 stat tiles + per-week table with rate bars + empty state. Verified via screenshot with seeded weeks (cleaned after).
- **Tests**: `tests/test_iter38_feature_pack.py` now 13 tests (fraud push 2, snooze RBAC 3, coverage planner 5, nudge stats 3). Full suite **501 passed, 7 skipped**. All UI flows self-verified via screenshot_tool (desktop + mobile).
- Backlog (P2): PayslipScanPanel pagination; sw.js refactor (offline caching + push routing in one file).

## v1.37 (Aug 2026) — iter39: Nudge Now, Scan Log pagination, Coverage Suggestions, Language Nudge Stats
- **Nudge Now Button**: "Send this week's nudges now" on ReminderNudgeStats header → `POST /api/training-progress/reminders/run-now` (existing, idempotent per ISO week) → toast with sent/reached/already-nudged/all-done counts + stats refresh. Verified E2E (sent 76 real reminders to gov tenant week 2026-W33 — rows intentionally left, they ARE this week's nudge).
- **Scan Log Pages**: PayslipScanPanel now paginates client-side, 20 rows/page (`payslip-scan-prev/next`, `payslip-scan-page-info`), footer hidden when ≤20 rows. Added an "All slips / Flagged only" filter (`payslip-scan-filter`) — filter change resets to page 1 and persists across page navigation; `?flag=` deep link auto-jumps to the page containing the flagged row. Verified with 32 seeded rows (cleaned).
- **Coverage Suggestions**: `coverage-preview` response now includes `suggestions` (up to 3) when warn=True — nearest same-duration date ranges (search ±60d, never before today, later-date preferred at equal distance) where NO day breaches the threshold. Both leave forms render clickable amber chips ("Nearby dates with full coverage") that apply the dates; verified E2E on desktop + mobile (chip click → dates applied → widget flips green).
- **Krio Nudge Stats**: reminder stats now carry per-language breakdown — top-level `languages` rollup AND per-week `weeks[].languages` (en/krio/mende/temne, always all 4: reminded, completed_after, nudge_rate). UI: 4-cell language strip between totals tiles and week table (`nudge-lang-{lang}`, native labels incl. Mɛnde, "—" when 0 sent).
- **Tests**: `test_iter38_feature_pack.py` now 18 tests (added TestCoverageSuggestions ×4, language breakdown ×1). Full suite **506 passed, 7 skipped**. All UI flows verified via screenshots.
